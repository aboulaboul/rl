...
if __name__ == "__main__":
    ...
    EPISODES_TIME = 200
    STEPS_TO_UPDT_MOD_TGT = 2000
    STATE_SIZE = env.observation_space.shape[0]
    ACTION_SIZE = env.action_space.n
    memory_replay_size = 4000
    batch_size = 2 ** 5
    exploration_rate = 1.0
    exploration_rate_decay = 0.95  # nb: 1/(1-B)
    ...
    def academy_reward(state, reward, done, step, max_dur):
        if done and step == (EPISODES_TIME - 1):
            reward = reward
        elif done and state[0,0] >= 0.5:
            reward = 200
        else:
            reward = reward
        return reward
